...
function pvs_counter_static_days($current , $current_length){
	global $wpdb; // Normal global wpdb

  // Database table
	$table_name_sample_counter = $wpdb->prefix . 'sample_counter';

	$query = $wpdb->get_var( // returns a single variable from the database

    // Use prepare to protect queries against SQL injection attacks
    // This function will accept the SQL query and the values to pass into the query as argument
    $wpdb->prepare("
			 SELECT SUM(access)
